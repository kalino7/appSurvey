<?php
  ...
  {
    header("Location: surveyForms.php");
?>
    <script>
        setTimeout(() => {
        window.location.href="surveyForms.php";
        }, 0000);
    </script>
<?php
  }
  else
  {
    ...
    $query= $db->query("SELECT * FROM survey WHERE id = '$surveyID' ");
    if($query->num_rows <= 0)
    {
        header("Location: surveyForms.php");
?>
...
<head>
    ...
    <title>survey Criteria | <?php echo $theme; ?></title>
